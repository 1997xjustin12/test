...
const YouMightAlsoLike = () => {
  ...
  return (
    <div>
      <h3 className="font-oswald text-sm uppercase tracking-wide text-char dark:text-ash mb-3">
        You Might Also Like
      </h3>
      {products.length === 0 ? (
...
const GrillProtectionSection = () => {
  return (
    <div className="w-full flex flex-col lg:flex-row bg-ash dark:bg-smoke border border-grate dark:border-white/10 transition-colors">
      {/* Content Section */}
      <div className="w-full lg:w-[calc(100%-240px)] p-4 lg:p-6">
        <h4 className="font-oswald font-bold text-sm uppercase tracking-tight text-char dark:text-ash">
        ...
        <div className="flex flex-col sm:flex-row mt-4 gap-4">
          ...
          <div className="flex relative justify-center sm:justify-start">
            <div className="bg-white dark:bg-stone-800 w-[140px] h-[100px] lg:h-[140px] flex relative items-center justify-center rounded-sm overflow-hidden border border-grate dark:border-white/10">
              <Image
          ...
          <div className="w-full sm:pl-4 flex flex-col gap-6">
            <ul className="list-disc list-inside marker:text-theme-600 text-sm font-medium text-char/70 dark:text-ash/60 font-sora">
            ...
            <div className="text-sm">
              <Link
                href="#"
                prefetch={false}
                className="flex items-center gap-2 underline text-char dark:text-ash hover:text-theme-600 dark:hover:text-theme-500 transition-colors font-sora"
              >
            ...
            {/* Features Row */}
            <div className="text-xs sm:text-sm flex flex-wrap gap-4 items-center text-char/50 dark:text-ash/40 font-sora">
      ...
      {/* Pricing Section */}
      <div className="w-full lg:w-[240px] p-4 lg:p-6 bg-ash dark:bg-white/5 flex flex-col items-center justify-center gap-3 border-t lg:border-t-0 lg:border-l border-grate dark:border-white/10">
        <button className="px-5 py-2.5 w-full rounded-sm font-oswald font-semibold text-sm uppercase tracking-wide text-theme-600 dark:text-theme-500 bg-white dark:bg-smoke border border-theme-600 dark:border-theme-600/60 hover:bg-theme-600/10 dark:hover:bg-theme-600/20 transition-all">
          5 Year - $159.99
        </button>
        <button className="px-5 py-2.5 w-full rounded-sm font-oswald font-semibold text-sm uppercase tracking-wide text-theme-600 dark:text-theme-500 bg-white dark:bg-smoke border border-theme-600 dark:border-theme-600/60 hover:bg-theme-600/10 dark:hover:bg-theme-600/20 transition-all">
          3 Year - $99.99
...
const Header = ({ data, onClose }) => {
  ...
  return (
    <div className="px-4 py-3 flex items-center justify-between border-b border-grate dark:border-white/10">
      {/* Success pill */}
      <div className="inline-flex items-center gap-1.5 bg-bbq-green/10 dark:bg-bbq-green/20 border border-bbq-green/30 dark:border-bbq-green/20 rounded-sm px-3 py-1 pl-1.5">
        <div className="w-[18px] h-[18px] rounded-sm bg-bbq-green flex items-center justify-center text-white flex-shrink-0">
          <CheckIcon cls="w-2.5 h-2.5" />
        </div>
        <span className="text-[11px] font-medium font-oswald uppercase tracking-wide text-bbq-green">
      ...
      <button
        ...
        className="w-7 h-7 rounded-sm border border-grate dark:border-white/10 bg-transparent flex items-center justify-center text-char/40 dark:text-ash/40 hover:bg-ash dark:hover:bg-white/5 transition-colors"
      >
...
const Savings = ({ price, was, quantity = 1, format = "number" }) => {
  ...
  return (
    <>
      {quantity === 1 ? (
        <span className="text-[11px] text-char/40 dark:text-ash/40 line-through">
          {formatPrice(was)}&nbsp;
        </span>
      ) : (
        <>
      ...
      <span className="text-[9px] font-oswald font-bold uppercase text-bbq-green bg-bbq-green/10 dark:bg-bbq-green/20 border border-bbq-green/30 dark:border-bbq-green/20 rounded-sm px-1.5 py-0.5">
        Save {saved}
      </span>
    </>
  );
...
function CartItem({ item }) {
  return (
    <div
      className="grid border-b border-grate dark:border-white/10"
    ...
    >
      ...
      <div className="relative overflow-hidden min-h-[100px]">
        ...
        <span className="absolute bottom-2 left-2.5 z-10 font-oswald text-[9px] uppercase tracking-widest text-white/30">
          {STORE_NAME}
        </span>
      </div>

      {/* Info */}
      <div className="border-l border-grate dark:border-white/10 p-3 flex flex-col gap-1 font-sora">
        <p className="font-oswald text-[9px] tracking-widest uppercase text-char/40 dark:text-ash/40">
          {item?.brand}
        </p>
        <p className="text-[12px] font-medium leading-snug text-char dark:text-ash">
          {item?.title}
        </p>
        ...
        <div className="flex flex-col mt-0.5">
          <div className="flex items-baseline gap-1.5 flex-wrap">
            <span className="font-oswald text-[17px] font-bold text-ember-deep dark:text-ember tracking-tight">
          ...
          {item?.quantity > 1 && (
            <div className="text-[12px] text-char/50 dark:text-ash/40">
        ...
        <div className="flex items-center gap-1.5 flex-wrap mt-1">
          <span className="font-oswald text-[10px] uppercase tracking-wide text-char/50 dark:text-ash/40 bg-ash dark:bg-white/5 border border-grate dark:border-white/10 rounded-sm px-2 py-0.5">
            Qty: {item?.quantity}
          </span>
        </div>

        {/* Shipping */}
        <div className={`flex items-center gap-1.5 text-[10px] font-medium mt-auto pt-1 ${item?.is_freeShipping ? "text-bbq-green" : "text-char/40 dark:text-ash/40 line-through"}`}>
          <TruckIcon />
...
function AddedToCartDialog({ data, onClose }) {
  ...
  return (
    <Dialog open={toggle} onClose={setToggle} className="relative z-30">
      <DialogBackdrop
        transition
        className="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity data-[closed]:opacity-0 data-[enter]:duration-[350ms] data-[leave]:duration-200 data-[enter]:ease-[cubic-bezier(0.16,1,0.3,1)] data-[leave]:ease-in"
      />
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto overflow-x-hidden">
        <div className="w-screen h-full relative">
          <div className="absolute top-10 left-0 right-0 flex items-end justify-center md:p-4 text-center sm:items-center sm:p-[10px]">
            <DialogPanel
              transition
              className="w-full relative transform overflow-hidden bg-white dark:bg-char rounded-sm text-left shadow-2xl shadow-char/20 dark:shadow-black/50 transition-all data-[closed]:translate-y-4 data-[closed]:opacity-0 data-[enter]:duration-[400ms] data-[leave]:duration-200 data-[enter]:ease-[cubic-bezier(0.16,1,0.3,1)] data-[leave]:ease-in sm:my-8 sm:w-full sm:max-w-[800px] data-[closed]:sm:translate-y-0 data-[closed]:sm:scale-95 overflow-y-auto"
            >
              <Header data={items} onClose={handleClose} />

              <div className="flex flex-col bg-paper dark:bg-smoke border-y border-grate dark:border-white/10 shadow-inner px-[15px] py-[15px] gap-[10px]">
                <AddedToCartProducts data={items} />
                <div className="flex justify-between items-center gap-[10px] w-full">
                  <Link
                    onClick={handleGoToCartClick}
                    href={cartPageUrl}
                    className="flex-1 py-2.5 text-center rounded-sm bg-theme-600 hover:bg-theme-700 text-white font-oswald font-semibold text-[13px] uppercase tracking-wide transition-all duration-200 hover:-translate-y-0.5 active:scale-[.98]"
                  >
                    Go to Cart
                  </Link>
                  <button
                    onClick={handleClose}
                    className="flex-1 py-2.5 rounded-sm border border-grate dark:border-white/10 text-char dark:text-ash font-oswald font-semibold text-[13px] uppercase tracking-wide hover:bg-ash dark:hover:bg-white/5 transition-colors"
                  >
                    Continue Shopping
                  </button>
                </div>
