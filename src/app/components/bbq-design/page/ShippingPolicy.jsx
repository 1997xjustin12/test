...
export default function BBQShippingPolicyPage() {
  return (
    <div className="max-w-[1240px] mx-auto px-4 sm:px-6 py-10 font-sora">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        {highlights.map((h) => (
          <div
            key={h.title}
            className="bg-paper dark:bg-smoke border border-grate dark:border-white/10 rounded-sm p-5 text-center hover:border-theme-600 dark:hover:border-theme-600/60 hover:-translate-y-0.5 transition-all duration-200"
          >
            <span className="text-2xl block mb-2">{h.icon}</span>
            <p className="font-oswald text-sm font-semibold uppercase tracking-wide text-char dark:text-ash">{h.title}</p>
            <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">{h.sub}</p>
          </div>
        ))}
      </div>
        ...
        <div className="space-y-4 lg:sticky lg:top-[120px] self-start">
          <InfoCard icon="📞" title="Shipping Questions?" accent>
            ...
            <Link
              prefetch={false}
              href={`tel:${contact}`}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-sm font-oswald font-semibold text-sm uppercase tracking-wide text-white bg-theme-600 hover:bg-theme-700 transition-colors"
            >
              {contact}
            </Link>
          </InfoCard>
            ...
            <ul className="space-y-2 text-xs">
              {[
                ...
                >
                  ...
                  <span className="font-semibold text-char dark:text-ash text-right max-w-[55%]">{v}</span>
                </li>
              ))}
            </ul>
          </InfoCard>
