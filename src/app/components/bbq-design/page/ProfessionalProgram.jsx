...
export default function BBQProfessionalProgramPage() {
  return (
    <div className="max-w-[1240px] mx-auto px-4 sm:px-6 py-10 font-sora">
      ...
      <div className="mb-12">
        <div className="text-center mb-8">
          ...
          <h2 className="font-oswald font-bold text-2xl sm:text-3xl uppercase tracking-tight text-char dark:text-ash mt-1">
            How It Works
          </h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {steps.map((s) => (
            <div key={s.n} className="flex gap-4">
              <span className="font-oswald text-3xl font-bold text-grate dark:text-white/10 shrink-0 select-none leading-none mt-0.5">
                {s.n}
              </span>
              <div>
                <p className="font-oswald font-semibold uppercase tracking-wide text-char dark:text-ash text-sm mb-1">{s.title}</p>
                <p className="text-sm font-light text-stone-500 dark:text-stone-400 leading-relaxed">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <PolicySection title="Who Qualifies">
            ...
            <p>
              ...
              <Link
                ...
                className="underline text-theme-600 hover:text-theme-700 transition-colors"
              >
                Give us a call
              </Link>{" "}
        ...
        <div className="space-y-4 lg:sticky lg:top-[120px] self-start">
          <InfoCard icon="📋" title="Ready to Apply?" accent>
            ...
            <Link
              prefetch={false}
              href={`tel:${STORE_CONTACT}`}
              className="flex items-center justify-center w-full py-2.5 rounded-sm font-oswald font-semibold text-sm uppercase tracking-wide text-white bg-theme-600 hover:bg-theme-700 transition-colors"
            >
              {STORE_CONTACT}
            </Link>
          </InfoCard>
          ...
          <InfoCard icon="⏱️" title="Approval Timeline">
            <ul className="space-y-2 text-xs">
              {[
                ...
                <li
                  key={k}
                  className="flex justify-between border-b border-grate dark:border-white/10 pb-1.5 last:border-0 last:pb-0"
                >
          ...
          <InfoCard icon="💬" title="Have Questions?">
            ...
            <Link
              ...
              className="flex items-center justify-center w-full py-2.5 rounded-sm font-oswald font-semibold text-sm uppercase tracking-wide border border-grate dark:border-white/10 text-char dark:text-ash hover:border-theme-600 hover:text-theme-600 dark:hover:text-theme-500 transition-colors"
            >
              Contact Us
            </Link>
          </InfoCard>
