...
import { STORE_NAME2, STORE_EMAIL, STORE_CONTACT } from "@/app/lib/store_constants";
...
import PolicySection from "@/app/components/bbq-design/utility/PolicySection";
...
export default function BBQPrivacyPolicyPage() {
  return (
    <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-10 font-sora">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        <div className="lg:sticky lg:top-[120px] self-start">
          <div className="bg-paper dark:bg-smoke border border-grate dark:border-white/10 rounded-sm p-5">
            <p className="font-oswald text-xs font-semibold uppercase tracking-[.14em] mb-3 text-theme-600">
              Table of Contents
            </p>
            <nav className="space-y-1">
              {toc.map((t, i) => (
                <a
                  key={t}
                  href={`#privacy-${i}`}
                  className="flex items-center gap-2 px-3 py-2 rounded-sm text-xs font-medium text-char/50 dark:text-ash/40 hover:bg-theme-600/5 dark:hover:bg-theme-600/10 hover:text-theme-600 dark:hover:text-theme-500 transition-colors"
                >
                  <span className="text-grate dark:text-white/20 font-mono text-[10px]">
        ...
        <div className="lg:col-span-3 bg-paper dark:bg-smoke border border-grate dark:border-white/10 rounded-sm p-6 sm:p-10">
          <p className="text-sm font-light leading-relaxed text-stone-600 dark:text-stone-400 mb-8 p-4 rounded-sm border-l-4 bg-theme-600/5 dark:bg-theme-600/10 border-theme-600">
          ...
          <div id="privacy-2">
            <PolicySection title="3. Information Sharing">
              ...
              <ul className="list-disc list-inside space-y-1 mt-2 ml-2">
                <li>
                  <strong className="text-char dark:text-ash">Service providers</strong>{" "}
                  — payment processors, shipping carriers, email platforms
                </li>
                <li>
                  <strong className="text-char dark:text-ash">Analytics providers</strong>{" "}
                  — to understand site usage (data is anonymized)
                </li>
                <li>
                  <strong className="text-char dark:text-ash">Legal authorities</strong>{" "}
            ...
            <PolicySection title="4. Cookies & Tracking">
              ...
              <div className="mt-3 space-y-2">
                {[
                  ...
                  <div
                    key={t}
                    className="flex items-start gap-3 p-3 rounded-sm border border-grate dark:border-white/10"
                  >
                    <div className="font-oswald text-xs font-semibold uppercase tracking-wide text-theme-600 px-2 py-0.5 bg-theme-600/10 dark:bg-theme-600/20 rounded-sm">
                      {t}
                    </div>
                    <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">{d}</p>
                  </div>
                ))}
              </div>
              <p className="mt-3">
          ...
          <div id="privacy-8">
            <PolicySection title="9. Contact Us">
              ...
              <div className="mt-3 p-4 rounded-sm border border-grate dark:border-white/10 space-y-1">
                <p className="font-oswald font-semibold uppercase tracking-wide text-char dark:text-ash">
