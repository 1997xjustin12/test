...
function FaqsUpdater() {
  ...
  const handleChange = (new_content) => {
    ...
    setTabs((prev) => {
      return prev.map((i) => ({
        ...i,
        content: i.key === tab ? new_content : i.content,
      }));
    });
  };
  ...
  const handleSave = () => {
    ...
    const shipping_policy_value = tabs.find(
      ({ key }) => key === shipping_policy
    )?.content;
    const return_policy_value = tabs.find(
      ({ key }) => key === return_policy
    )?.content;
  ...
  useEffect(() => {
    redisGet([about, shipping_policy, return_policy, warranty]).then((res) => {
      setTabs([
        { key: about, label: `About ${STORE_NAME}`, content: res[0] },
  ...
  return (
    <CardWrap>
      <div className="p-3">
        ...
        <div className="mt-5">
          <Button loading={isLoading} disabled={isLoading} onClick={handleSave}>
            Save
          </Button>
        </div>
      </div>
