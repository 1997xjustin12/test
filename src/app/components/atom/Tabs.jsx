...
`,
  },
  {
    ...
    content: `<p>At ${STORE_NAME}, we strive to provide a smooth and hassle-free shopping experience for our customers. We understand that costs can add up, so we offer free shipping on all orders over $79.99 to make your purchase even more budget-friendly.</p>
...
`,
  ...
  },
  {
    key: "warranty",
    name: "warranty",
    content: `If you have an issue with a product we are happy to assist with the warranty claim.`,
  },
];
function Tabs() {
  ...
  return (
    <div>
      <div className="block  lg:flex lg:items-center">
        {tabs.map((i) => (
          <div key={`tab-button-${i.key}`} className="w-full">
            <button
              onClick={() => handleTabChange(i.key)}
              className={`w-full text-sm uppercase font-semibold border-b-2 py-[20px] transition-all duration-700 ease-in-out ${
                tab === i.key
                  ? "text-stone-950 border-theme-700"
                  : "text-stone-600 "
              }`}
            >
              {i.name}
            ...
            {i.key === tab && (
              <div className=" lg:hidden p-3 text-xs">{parser(i?.content)}</div>
            )}
          </div>
        ))}
      </div>
