...
// Full-color card-network logos need a light chip behind them to read correctly.
const CARD_LOGOS = {
  Visa: "/icons/payments/visa.svg",
  Mastercard: "/icons/payments/mastercard.svg",
  Amex: "/icons/payments/amex.svg",
  PayPal: "/icons/payments/paypal.svg",
};

// Apple Pay / Google Pay brand guidelines require a monochrome mark, so these
// render as inline currentColor icons instead of a colorful static image.
const AppleIcon = (props) => (
  <svg role="img" viewBox="0 0 24 24" fill="currentColor" aria-label="Apple Pay" {...props}>
...
const GoogleIcon = (props) => (
  <svg role="img" viewBox="0 0 24 24" fill="currentColor" aria-label="Google Pay" {...props}>
...
export default function PaymentIcons({ methods = [], className = "", walletIconClassName = "text-white/70" }) {
  return (
    <div className={`flex flex-wrap items-center gap-1.5 ${className}`}>
      {methods.map((name) => {
        const logo = CARD_LOGOS[name];
        if (logo) {
          return (
            <span
              key={name}
              title={name}
              className="bg-white rounded px-1 py-1 flex items-center justify-center"
            >
              <Image src={logo} alt={name} width={32} height={20} className="h-3.5 w-auto" />
            </span>
          );
        }
        const WalletIcon = WALLET_ICONS[name];
        if (WalletIcon) {
          return (
            <span
              key={name}
              title={name}
              className={`flex items-center justify-center h-6 w-6 ${walletIconClassName}`}
            >
