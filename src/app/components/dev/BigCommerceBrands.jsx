...
function BigCommerceCategories() {
  const API_URL = `${BASE_URL}/api/brands`; // Replace with your API
  ...
  return (
    <CardWrap>
      ...
      <div className="flex items-center gap-[10px]">
          ...
          <div>
            <div className="text-sm font-semibold">Bigcommerece Brands</div>
            <div className="text-xs">Get all bigcommerce brands.</div>
          </div>
        </div>
        <div>
          <button
            onClick={() => setShowResults((prev) => !prev)}
            className="border p-2 rounded hover:border-stone-300"
            title="Show or Hide Results"
          >
            {showResults && (
              <svg
