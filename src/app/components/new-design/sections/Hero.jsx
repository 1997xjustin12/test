        ...
        ">
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              <a href="#categories" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg bg-fire hover:bg-fire-light text-white font-semibold transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-fire/30">
                Shop All Products
              </a>
              <a href={PHONE_HREF} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg border-2 border-white/60 text-white hover:bg-white/10 font-semibold transition-all duration-200">
                Get Expert Advice
              </a>
            </div>
            ...
            <div className="flex gap-8 mb-5">
              {STATS.map(({ num, label }) => (
                <div key={label}>
                  ...
                  <div className="text-[11px] tracking-widest uppercase text-white/40 mt-0.5">{label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              {TRUST.map(t => (
                <div key={t} className="flex items-center gap-1.5 text-white/55 text-xs before:content-['✓'] before:text-fire-light before:font-bold">
                  {t}
                </div>
              ))}
            </div>
          ...
          <div ref={cardsRef} className="hidden md:flex flex-col gap-3 opacity-0 translate-y-6 transition-all duration-700 delay-150">
            {CARDS.map(({ title, sub }) => (
              <a key={title} href="#" className="
              ...
              ">
                <div className="h-40 bg-gradient-to-br from-stone-900 to-stone-800" />
                <div className="px-5 py-3.5 flex items-center justify-between">
                  <div>
                    <h3 className="text-white text-[15px] font-semibold">{title}</h3>
                    <p className="text-white/50 text-xs mt-0.5">{sub}</p>
                  </div>
                  <div className="w-8 h-8 rounded-lg bg-fire flex items-center justify-center text-white text-sm group-hover:bg-fire-light transition-colors">→</div>
                </div>
              </a>
            ))}
          </div>
