...
function computeBars(reviews) {
  if (!reviews?.length) return [5, 4, 3, 2, 1].map((star) => ({ star, pct: 0 }));
        ...
        <div className="p-5">
          <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-6 items-center pb-6 border-b border-gray-100 dark:border-gray-800 mb-6">
            <div className="text-center sm:pr-6 sm:border-r sm:border-gray-100 dark:sm:border-gray-800">
              <p className="text-6xl font-black text-gray-900 dark:text-white leading-none">{rating > 0 ? rating.toFixed(1) : "—"}</p>
            ...
            <div className="flex flex-col divide-y divide-gray-100 dark:divide-gray-800">
              {reviews.map((review, i) => (
                <div key={review.id ?? i} className="py-5 first:pt-0">
                  ...
                  {review.title && (
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-1">{review.title}</p>
                  )}
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{review.body || review.content || ""}</p>
                </div>
              ))}
            </div>
          ) : (
